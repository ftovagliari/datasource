 ...
 * $Id: pGOCaml.ml,v 1.12 2006/02/19 22:51:17 rich Exp $
 *)
...
let add_string_no_trailing_nil (buf, _) str =
  ...
  if String.contains str '\000' then
    raise (Error "PGOCaml: string contains ASCII NIL character.");
...
let parse_backend_message (typ, msg) =
  ...
  let get_char where =
    if !pos < len then begin
      ...
      incr pos;
      r
    end else raise (Error ("PGOCaml: parse_backend_message: " ^ where ^ ": short message"))
  in
...
let connect ?host ?port ?user ?(password = "") ?database
  ...
  (* Get the username, using $PGUSER, $LOGNAME, $USERNAME, $USER
   * and "postgres" in that order.
   *)
  ...
  let user =
	...
	with Not_found ->
	  try Sys.getenv "LOGNAME"
	  with Not_found ->
	    try Sys.getenv "USERNAME"
	    with Not_found ->
	      try Sys.getenv "USER"
	      with Not_found -> "postgres" in
  ...
  let conn = { ichan = ichan; chan = chan; private_data = None } in
...
let rec next rs =
  ...
  let msg = parse_backend_message msg in
  match msg with
    | ReadyForQuery _ -> raise Ready_for_query (* Finished! *)
    | ErrorResponse err -> pg_error ~conn err (* Error *)
...
let timestamp_of_string = Printer.CalendarPrinter.from_string
let date_of_string = Printer.DatePrinter.from_string
let time_of_string = Printer.TimePrinter.from_string
